...
class Berita extends CI_Controller
{
	...
	public function cari()
	{
		...
		// Berita dan paginasi
		$this->load->library('pagination');
		$config['base_url'] 		= base_url() . 'berita/cari?s=' . $keywords . '/index/';
	...
	public function kategori($slug_kategori)
	{
		...
		$kategori_list 	= $this->nav_model->nav_kategori_berita();
		$populer	= $this->berita_model->populer();
	...
	{
		$site 		= $this->konfigurasi_model->listing();
		$berita 	= $this->berita_model->profil();
		$profil 	= $this->nav_model->nav_profil();

		if ($id != NULL)
		{
			redirect(base_url('berita/profil'));
		}
	...
	public function layanan($id = NULL)
	{
		...
		if ($id != NULL)
		{
			redirect(base_url('berita/layanan'));
		}
		...
		$this->pagination->initialize($config);
		$page 		= ($this->uri->segment(3)) ? ($this->uri->segment(3) - 1) * $config['per_page'] : 0;
		$berita 	= $this->berita_model->layanan($config['per_page'], $page);
		...
		$data = array(
			...
			'berita'	=> $berita,
			'site'		=> $site,
			'isi'		=> 'berita/list_layanan'
		);
		$this->load->view('layout/wrapper', $data, FALSE);
	}

	// Layanan detail
	public function baca_layanan($slug_berita)
	{
		$site 		= $this->konfigurasi_model->listing();
		$berita 	= $this->berita_model->read($slug_berita);
